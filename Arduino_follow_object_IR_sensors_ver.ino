...
void turnRight() {
  ...
  digitalWrite(motor2Pin1, HIGH);
  digitalWrite(motor2Pin2, LOW);  // Right motor backward
}
...
void loop() {
  ...
  if (leftRead == 0 && rightRead == 1) { // Turn left if object within range of left
    ...
    turnRight();
  } else if (frontDist < 5) { // Move backward if too close
    moveBackward();
  } else if (frontDist >= 10 && frontDist <= 50) { // Move forward if object within certain range of front
    moveForward();
  } else {
    stopMoving();
  }
